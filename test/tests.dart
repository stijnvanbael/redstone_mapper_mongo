...
main() {
  ...
  test("Decode", () {
    mockCol
        .when(callsTo("find"))
        .alwaysReturn(new MockCursor([userMap, userMap, userMap]));
  ...
  test("Encode: Update", () {
    ...
    mockCol
        .when(callsTo("update"))
        .alwaysCall((selector, document, {upsert, multiUpdate, w}) {
    ...
    return mongoDb
        .update(mongoDb.collection("user"), userMap, userObj)
        .then((_) {
  ...
  group("MongoDbService:", () {
    ...
    test("save", () {
      ...
      var req = new MockRequest("/save",
          method: POST, bodyType: JSON, body: userJson);
      return dispatch(req).then((resp) {
